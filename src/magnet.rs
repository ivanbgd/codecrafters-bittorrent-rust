...
pub async fn magnet_handshake(magnet_link: &str) -> Result<Peer, MagnetError> {
    ...
    eprintln!(" xt : {}", magnet_link.xt); //todo
    let info_hash = hex::decode(magnet_link.xt.clone())?;
    eprintln!(" inf: {:?}", info_hash); //todo
    let info = crate::tracker::url_encode(&info_hash);
    eprintln!(" info: {:?}", info); //todo
    let mut buf: HashType = [0; SHA1_LEN];
    buf.copy_from_slice(&info_hash);
    eprintln!(" buf: {:?}", buf); //todo
                                  // let info_hash = <HashType>::try_from(info_hash)?; // todo
                                  // let info_hash: HashType = info_hash.try_into()?; // todo

    // Perform the tracker GET request to get a list of peers.
    let peers = get_peers(magnet_link.tr.as_deref(), info).await?;
...
async fn get_peers(tracker: Option<&str>, info_hash: String) -> Result<Peers, MagnetError> {
    ...
    eprintln!("**info_hash: {info_hash}"); // todo rem
