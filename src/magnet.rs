...
pub async fn magnet_handshake(magnet_link: &str) -> Result<Peer, MagnetError> {
    ...
    let info_hash = magnet_link.xt.into_bytes();
    let buf: HashType = [0; SHA1_LEN];

    let mut peer = Peer::new(&"165.232.41.73:51556".parse().unwrap());
...
// fn get_peers() -> Result<(Peers, Info), MagnetError> {}
